...
contract BlockCom {
    ...
    // Create a new buyer
    function createBuyer(string memory _name, string memory _shippingAddress)
        public
    {
    ...
    // Create a new order
    function createOrder(uint256 _id) public payable {
        ...
        PlacedOrders memory placedOrder = PlacedOrders(
            _id,
            orderCount,
            "Order Placed",
            msg.sender,
    ...
    function updateOrderStatus(uint256 _index, string memory _newStatus)
        public
    {
    ...
    function getBuyerOrders(uint256 _index)
        public
        view
        returns (
            uint256,
        ...
        )
    {
        return (
            ...
            UserOrders[msg.sender][_index].orderId,
            UserOrders[msg.sender][_index].status
        );
    }

    // Return all orders recieved by a seller
    function getSellerOrders(uint256 _index)
        public
        view
        returns (
            uint256,
        ...
        )
    {
        return (
            ...
            SellerOrders[msg.sender][_index].orderId,
            SellerOrders[msg.sender][_index].status,
            SellerOrders[msg.sender][_index].buyer
        );
    }
}
