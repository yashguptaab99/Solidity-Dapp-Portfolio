...
contract PropertyExchange is PxCoinFactory, Pausable {
    ...
    modifier onlyRegistered() {
        require(
            users[msg.sender].isRegistered || escrows[msg.sender].isRegistered,
            "Only registered users can call this"
        );
        _;
    }
    ...
    modifier onlyOwner(uint256 _coinId) {
        require(
            ownerOf(_coinId) == msg.sender,
    ...
    modifier isOwedToEscrow(address payable _to) {
        require(
            escrows[_to].isRegistered == true,
            "Only allowed to transfer to escrow"
        );
        require(
            escrows[_to].agreements[msg.sender].owed > 0,
            "You do not owe the escrow anything"
        );
        _;
    }
    ...
    modifier requireSignedByBothParties(address _escrow, address _buyer) {
        ...
        require(
            _agreement.signedByBuyer && _agreement.signedBySeller,
    ...
    modifier requireEscrowBuyerRelationship(address _escrow, address _buyer) {
        require(
            escrows[_escrow].agreements[_buyer].buyer == _buyer,
            "No escrow relationship exists"
        );
        _;
    }
    ...
    event LogPropertyAdded(
        uint256 indexed _propertyId,
        uint256 _coinId,
        uint256 _value
    );
    event LogAgreementSigned(
        address indexed _user,
        uint256 _coinId,
        address _owner
    );
    event LogTransferToEscrow(
        address indexed _from,
        address _escrow,
        uint256 _amount
    );
    event LogPropertyTransfer(
        address indexed _from,
        address _to,
        uint256 _coinId
    );
    ...
    function register(bool _isEscrow) public whenNotPaused {
        if (_isEscrow) {
            require(
                ...
                "already registered as user, cannot be an escrow"
            );
            require(
                escrows[msg.sender].isRegistered == false,
                "already registered as escrow"
            );
            escrows[msg.sender].isRegistered = true;
        } else {
            require(
                escrows[msg.sender].isRegistered == false,
                "already registered as escrow, cannot be a user"
            );
            require(
                users[msg.sender].isRegistered == false,
     ...
     */
    function addNewProperty(uint256 _propertyId, uint256 _value)
        public
        payable
        whenNotPaused
        onlyUser
    {
        uint256 _coinId = mint(_propertyId, _value);
    ...
    function addEscrow(address _escrow) public payable whenNotPaused onlyUser {
        require(
            escrows[_escrow].agreements[msg.sender].buyer == address(0),
            "buyer escrow relationship already exists"
        );
        ...
        requireEscrowBuyerRelationship(_escrow, _buyer)
    {
        ...
        if (_ownerId == msg.sender) {
            ...
            require(
                msg.sender == _buyer,
                "Buyer address doesn't match the caller"
            );
     ...
     */
    function _refundExcess(uint256 target, uint256 given)
        public
        payable
        whenNotPaused
        returns (uint256)
     ...
     */
    function transferEth(address payable _to)
        public
        payable
        whenNotPaused
        onlyUser
        ...
        isOwedToEscrow(_to)
    {
        uint256 _owed = escrows[_to].agreements[msg.sender].owed;

        uint256 _paid = escrows[_to].agreements[msg.sender].paid;

        uint256 _balance = _owed - _paid;

        uint256 _amountToBePaid = _refundExcess(_balance, msg.value);
    ...
    {
        require(
            escrows[msg.sender].agreements[_to].owed <=
                escrows[msg.sender].agreements[_to].paid,
            "Insufficient balance to buy the property"
        );
        require(
            escrows[msg.sender].agreements[_to].owed <= msg.value,
            "msg.value should be >= property's sale price"
        );

        uint256 _owed = escrows[msg.sender].agreements[_to].owed;

        // refund excess amount provided as msg.value
        uint256 _amountToBePaid = _refundExcess(_owed * 1 ether, msg.value);
